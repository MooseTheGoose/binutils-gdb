  ...
  struct mos65xx_arg_widths widths, int addrmode)
{
  ...
  for(i = 0; i < sizeof(exps) / sizeof(exps[0]); i++)
  {
    ...
    {
      printf("%d, %d\n", MOS65XX_SIZEOF(X_md), width);
      as_fatal("Forcing width of operand led to invalid addressing mode for instruction");
    }
...
md_assemble(char *line)
{
  ...
  if(MOS65XX_SIZEOF(operand.lhs.X_md) != 0)
  {
    default_width = MOS65XX_SIZEOF(operand.lhs.X_md);
  }
  else if(operand.lhs.X_op == O_constant)
  {
...
tc_gen_reloc(asection *seg ATTRIBUTE_UNUSED, fixS *fixp ATTRIBUTE_UNUSED)
{
  ...
  *reloc->sym_ptr_ptr = symbol_get_bfdsym(fixp->fx_addsy);
  ...
  /* ELF needs to subtract size of relocation in addend for pcrel */
  if(fixp->fx_pcrel)
